...
public class ServerVierGewinnt extends Server
{
	...
	private void messageReaction(Message pMessageData, User pAbsender)
	{
		...
		{
			case GAME:
				User user = myUserControl.getUser(pAbsender);
				if(user != null)
				{
					if (pMessageData.command == MessageCommand.INSERT)
					{
						try
						{
							...
							{
								myGameHandler.einwurf(spalte, user, chipType);
							} else
							{
					...
					} else if (pMessageData.command == MessageCommand.EXPLOSION)
					{
						try
						{
							GameControl game = myGameHandler.getGame(user);
							...
							if (game != null)
							{
								...
								game.sprengen(spalte, zeile, user);
							} else
							{
								// Spiel nicht gefunden
								fehler("messageReaction.Explosion: Spiel nicht gefunden (Teilnehmer : " + user + ")");
							}
						} catch (Exception e)
						{
							// Einlesefehler
					...
					{
						fehler("messageReaction.Game: Spieler nicht gefunden: " + pAbsender);
					}
				}
				break;
